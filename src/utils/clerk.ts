...
export async function syncDataToDatabase(userId: string, user: User, organizations: { data: OrganizationMembership[] }) {
	try {
		// Sync user data
		...
		await Promise.all([
			...
			(async () => {
				...
				await db.delete(organizationMembersTable).where(and(eq(organizationMembersTable.userId, userId), isNull(inArray(organizationMembersTable.organizationId, currentOrgIds))));
		...
		if (orgsWithoutMembers.length > 0) {
			await db.delete(organizationsTable).where(
				inArray(
					...
					orgsWithoutMembers.map((org) => org.id)
				)
			);
		}
	} catch (error) {
		console.error("Error syncing data to database");
		throw error;
	}
}
