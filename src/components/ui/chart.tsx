							...
							{formatter && item?.value !== undefined && item.name ? (
								...
								<>
									...
									<div className={cn("flex flex-1 justify-between leading-none", nestLabel ? "items-end" : "items-center")}>
										...
										{item.value && <span className="font-mono font-medium tabular-nums text-neutral-950 dark:text-neutral-50">{item.value.toLocaleString()}</span>}
									</div>
								</>
							)}
						</div>
	...
	return (
		<div ref={ref} className={cn("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className)}>
			{payload.map((item) => {
				...
				return (
					<div key={item.value} className={cn("flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-neutral-500 dark:[&>svg]:text-neutral-400")}>
